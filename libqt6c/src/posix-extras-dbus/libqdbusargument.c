...
QDBusArgument* q_dbusargument_operator_shift_left15(void* self, const char* arg[1]) {
...
const QDBusArgument* q_dbusargument_operator_shift_right15(void* self, const char* arg[1]) {
    size_t arg_len = libqt_strv_length(arg);
    libqt_string* arg_qstr = (libqt_string*)malloc(arg_len * sizeof(libqt_string));
    if (arg_qstr == NULL) {
