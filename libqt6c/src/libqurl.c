...
libqt_list /* of QUrl* */ q_url_from_string_list(const char* uris[1]) {
...
void q_url_set_idn_whitelist(const char* idnWhitelist[1]) {
...
libqt_list /* of QUrl* */ q_url_from_string_list2(const char* uris[1], int32_t mode) {
    size_t uris_len = libqt_strv_length(uris);
    libqt_string* uris_qstr = (libqt_string*)malloc(uris_len * sizeof(libqt_string));
    if (uris_qstr == NULL) {
