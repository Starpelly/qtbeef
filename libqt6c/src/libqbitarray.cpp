...
unsigned int QBitArray_ToUInt32(const QBitArray* self, QSysInfo::Endian endianness) {
    return static_cast<unsigned int>(self->toUInt32(endianness));
}
...
unsigned int QBitArray_ToUInt322(const QBitArray* self, QSysInfo::Endian endianness, bool* ok) {
    return static_cast<unsigned int>(self->toUInt32(endianness, ok));
}
