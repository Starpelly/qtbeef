...
class QStyledItemDelegate : IQStyledItemDelegate, IQAbstractItemDelegate, IQObject
{
	...
	{
		return CQt.QStyledItemDelegate_SizeHint((.)this.ptr, (.)option?.ObjectPtr, (.)index?.ObjectPtr);
	}
	public QWidget_Ptr** CreateEditor(IQWidget parent, IQStyleOptionViewItem option, IQModelIndex index)
	{
		return CQt.QStyledItemDelegate_CreateEditor((.)this.ptr, (.)parent?.ObjectPtr, (.)option?.ObjectPtr, (.)index?.ObjectPtr);
	}
	...
	{
		return CQt.QStyledItemDelegate_DisplayText((.)this.ptr, (.)value?.ObjectPtr, (.)locale?.ObjectPtr);
	}
